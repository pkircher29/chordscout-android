...
- **100% Local & Offline DSP:**
  ...
  - Chord template matching by centered (Pearson) correlation, decoded with Viterbi over the whole track. Each chord change costs a fixed penalty (0.9), and a new chord is kept only when its lead, summed over every frame it lasts, pays that off. Sustained changes land on the frame where the evidence flips; near-ties and passing tones never add up. This replaces the old per-frame hold/margin rules, which traded missed changes against flicker. Silence RMS (0.015), hop (1024) and minimum segment (0.10 s) still match the desktop app. The no-chord cutoff is 0.20 on the centered score (desktop's 0.35 is a cosine score and would blank out chords in a full mix), and a 3-frame chroma median removes single-frame flicker.
